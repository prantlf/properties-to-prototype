...
export function removeClassProperties(declaration, prototypePropertyNames,
  convertToPropertyGetters, prototypeDecorator, removePrototypeDecorator,
  instanceDecorator, removeInstanceDecorator) {
  ...
  for (i = 0; i < length; ++i) {
    ...
    if (statement.type === 'PropertyDefinition') {
      ...
      if (keepOrMove === undefined) {
        const { type, name } = key
        keepOrMove = type === 'Identifier' && prototypePropertyNames.includes(name)
      }
      if (keepOrMove) {
