...
export function updateClassDeclarations(program, options) {
  ...
  function handleClassDeclaration(declaration) {
    const [classUpdated, classType] = processClassDecorator(declaration, classDecorator, removeClassDecorator)
    const prototypePropertyNames = prototypeProperties[classType] ?? []
    const [className, removedProperties, propertiesUpdated] = removeClassProperties(
      declaration, prototypePropertyNames, convertToPropertyGetters,
